...
if ($CONFFILE) { 
	...
	open my $CONFIG, '<', $CONFFILE or die "Unable to open config file $CONFFILE $!";
	while(<$CONFIG>) {
		chomp;
   	if ( $_ =~ m/^[a-zA-Z]/) {
...
unless ( -d "$SDIR" ) {
	mkdir "$SDIR" or die("Session dir $SDIR doesn't exist, and I failed to create it");
	warn "[-] Created session dir $SDIR\n";
} else {
	unless ( -w "$SDIR" ) {
		die("Session directory $SDIR isn't writable");
	}
}
# If Failed subfolder doesn't exist, lets add it
unless ( -d "$FDIR" )  {
	mkdir "$FDIR" or die("Unable to create failed instertion dir $FDIR");
