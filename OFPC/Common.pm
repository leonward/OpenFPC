...
sub donode {
    ...
    if ( $r->{'stime'}{'val'} && $r->{'etime'}{'val'} ) {
        wlog( "NODE : Getting a bunch of pcap files between $r->{'stime'}{'val'} (" . localtime( $r->{'stime'}{'val'} ) . ") and $r->{'etime'}{'val'} (" . localtime( $r->{'etime'}{'val'} ) . ")" ) if $debug;
                            ...
                            if ( "$request->{'action'}{'val'}" eq "store" ) {
                                ...
                                if ( $prep->{'success'} ) {
                                    ...
                                    unless ( $config{'KEEPFILES'} ) {
                                        ...
                                        unlink("$config{'SAVEDIR'}/$xferfile")
                                          or wlog("COMMS: ERROR: $client_ip Request: $request->{'metadata'}{'rid'} : Unable to unlink $config{'SAVEDIR'}/$xferfile");
                                    }
                                }
                                else {
...
sub readconfig {
    ...
    if ( $config{'PROXY'} ) {
        ...
        # Proxy device
        @ce = ( 'PROXY_DB_USER', 'PROXY_DB_PASS', 'PROXY_DB_HOST', 'PROXY_DB_NAME', );

    }
    else {
        # normal node
        @ce = ( 'DESCRIPTION', 'BUFFER_PATH', );
    }
