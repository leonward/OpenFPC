...
sub parselog {
    ...
    my %eventdata = ( parsed => 0, );    # Hash of decoded event

    # Work through a list of file-parsers until we get a hit
    while (1) {
        %eventdata = OFPC::Parse::OFPC1Event( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::passive_dns_1( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::SF49IPS( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::Exim4( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::SnortSyslog( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::SnortFast( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::pradslog( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::nftracker( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::cxsearch( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::ofpc_search( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::generic_with_ports( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
        %eventdata = OFPC::Parse::generic_without_ports( $logline, $r->{'tz'}{'val'} );
        if ( $eventdata{'parsed'} ) { last; }
...
sub nftracker {
    ...
    ( $event{'timestamp'}, my $proto, $event{'sip'}, $event{'spt'}, $event{'dip'}, $event{'dpt'}, my $filetype ) = split( /,/, $logline );
